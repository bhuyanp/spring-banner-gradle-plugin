package io.github.bhuyanp.gradle.ansi;
...
public abstract class Attribute {
    ...
    String getReferrer(){
        return "%s()".formatted(Thread.currentThread().getStackTrace()[3].getMethodName());
    }


    String getReferrer(int colorNumber){
        return "%s(%s)".formatted(Thread.currentThread().getStackTrace()[3].getMethodName(),colorNumber);
    }

    String getReferrer(int r, int g, int b){
        return "%s(%s, %s, %s)".formatted(Thread.currentThread().getStackTrace()[4].getMethodName(),r, g, b);
    }

    /**
     * @return The Attribute original construct.
     */
